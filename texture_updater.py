...
def save_conf():
    _ = f"{os.path.splitext(os.path.split(meta_path)[1])[0]}.conf"
    _dat = [meta_path, tga_path, home_dir, ",".join(map(str, (sel_pos, follow_selector))), ",".join(map(str, scene_cp + [scale])), ",".join(tex_repl)]
...
try:
    with open(_conf, "r", encoding="utf-8") as inp:
        ...
        home_dir = rpath(inp.readline().strip())
...
def dbg_fill(is_valid: bool, spos: int = -1):
    spos = sel_pos if spos == -1 else spos
    color = (0, 0, 0) if is_valid else (0, 0, 0, 180)
    dbg_surf.fill(color, tex_pos[spos][0:2] + [tw, th])


def upd_fill(path: str = "", spos: int = -1):
    spos = sel_pos if spos == -1 else spos
    upd_surf.fill((0, 0, 0, 0), tex_pos[spos][0:2] + [tw, th])
        ...
        match ev.type:
            ...
            case pg.KEYDOWN:
                ...
                if ev.key == pg.K_RETURN:
                    ...
                    for i in _pos:
                        _pth = os.path.join(home_dir, tex_name[tex_pos[i][-1]])
                        _pth = _pth + (".png" if os.path.exists(_pth + ".png") else ".tga")
                        tex_repl[i] = _pth
